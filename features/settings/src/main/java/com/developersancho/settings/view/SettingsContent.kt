    ...
    Box(
        ...
        Card(
            ...
            ConstraintLayout(
                ...
                val viewDivider7 = createRef()
                val lblMaps = createRef()
                ...
                JRDivider(
                    modifier = Modifier
                        .padding(top = 12.dp, bottom = 12.dp)
                        .constrainAs(viewDivider7) {
                            start.linkTo(parent.start)
                            end.linkTo(parent.end)
                            top.linkTo(lblAppVersion.bottom)
                            height = Dimension.fillToConstraints
                        }
                )
                Text(
                    text = /*stringResource(id = R.string.maps)*/"Maps",
                    style = JetRortyTypography.body2,
                    modifier = Modifier.constrainAs(lblMaps) {
                        top.linkTo(viewDivider7.bottom)
                        bottom.linkTo(parent.bottom)
                        start.linkTo(parent.start)
                    }.clickableSingle {
                        navigator?.openMaps("hello world")
                    }
                )
            }
        }
